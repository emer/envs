...
// CondEnv provides a flexible implementation of standard conditioning experiments
// involving 
type CondEnv struct {
	Nm                 string                  `desc:"name of this environment"`
	Dsc                string                  `desc:"description of this environment"`
	NUSs int `desc:"number of USs"`
...
func (ev *CondEnv) New(ss *Sim) {
	...
	ev.TsrStimIn.SetShape([]int{12, 1}, nil, nil)
	ev.TsrContextIn.SetShape([]int{20, 3}, nil, nil)
	ev.TsrUSTimeIn.SetShape([]int{16, 2, 4, 5}, nil, nil)
	ev.TsrPosPV.SetShape([]int{ss.InputShapes["PosPV"], nil, nil)
	ev.TsrNegPV.SetShape(ss.InputShapes["NegPV"], nil, nil)
}

// From looking at the running cemer model, the chain is as follows:
// RunParams(=pos_acq) -> ConditionParams(=pos_acq_b50) -> CondEnv.vars["env_params_table"](=PosAcq_B50)
...
func (ev *CondEnv) Init(ss *Sim, firstCondition bool) (ok bool) {
	ev.CurConditionParams = ss.ConditionParams
...
func (ev *CondEnv) ConfigStdInputData(dt *etable.Table) {
...
// BlockStart
func (ev *CondEnv) BlockStart(ss *Sim) {
...
// BlockEnd
func (ev *CondEnv) BlockEnd(ss *Sim) {
	//ss.TrialAnalysis(ev)
	ss.BlockMonitor()
...
// SaveWeights
func (ev *CondEnv) SaveWeights(_ *Sim) {
...
func (ev *CondEnv) State(Nm string) etensor.Tensor {
	switch Nm {
	case "StimIn":
		return &ev.TsrStimIn
	case "ContextIn":
...
func (ev *CondEnv) SetActiveTrialList(ss *Sim) {
...
func (ev *CondEnv) SetTableTrialGpListFmDefnTable() {
	nRepeatsF := 0.0
	nRepeats := 0
	usFlag := false
...
func (ev *CondEnv) SetupOneAlphaTrial(curTrial *data.TrialInstance, stimNum int) {
	prefixUSTimeIn := ""
...
func (ev *CondEnv) IsTestTrial(ti *data.TrialInstance) bool {
