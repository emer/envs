...
type FWorld struct {
	...
	// list of materials in the world, 0 = empty.  Any superpositions of states (e.g., CoveredFood) need to be discretely encoded, can be transformed through action rules
	...
	// list of interoceptive body states, represented as pop codes
	Inters []string `desc:"list of interoceptive body states, represented as pop codes"`

	// map of interoceptive state names to indexes
	...
	// for debugging only: show the main depth rays as they are traced out from point
	ShowRays bool `desc:"for debugging only: show the main depth rays as they are traced out from point"`

	// for debugging only: show the fovea rays as they are traced out from point
	...
	// number of items on each size of the fovea, in addition to center (0 or more)
	FoveaSize int `desc:"number of items on each size of the fovea, in addition to center (0 or more)"`

	// scan angle for fovea
